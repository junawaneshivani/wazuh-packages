...
# Install migration.py file for migrating old API configuration
install -m 0640 api/migration.py ${RPM_BUILD_ROOT}%{_localstatedir}/ossec/api/migration.py
...
if [ $1 = 1 ]; then
  ...
  # Backup old API (<=3.9) configuration
  API_PATH=%{_localstatedir}/api
  API_PATH_BACKUP=%{_localstatedir}/~api

  # do backup only if config.js file exists
  if [ -f ${API_PATH}/configuration/config.js.rpmsave ]; then
    
    if [ -e ${API_PATH_BACKUP} ]; then
      rm -rf ${API_PATH_BACKUP}
    else
      install -m 0770 ${API_PATH} ${API_PATH_BACKUP}
      chown root:ossec ${API_PATH_BACKUP}
    fi
    
    install -m 0770 ${API_PATH_BACKUP}/configuration
    cp -rLfp ${API_PATH}/configuration/config.js ${API_PATH_BACKUP}/configuration/config.js
    
    if [ -d ${API_PATH}/configuration/ssl ]; then
      install -m 0770 ${API_PATH_BACKUP}/configuration/ssl
...
%attr(750, root, ossec) %{_localstatedir}/ossec/api/migration.py
%attr(750, root, ossec) %{_localstatedir}/ossec/bin/wazuh-apid
%attr(750, root, ossec) %{_localstatedir}/ossec/api/configuration/template-api.yml
